# FlowCreditMarket Refactoring Plan - Health/Withdraw/Liquidation Vertical Slice
...
access(all) struct PositionView {
    access(all) let balances: {Type: FlowCreditMarket.InternalBalance}     // copy
    ...
    init(balances: {Type: FlowCreditMarket.InternalBalance},
...
access(all) fun effectiveCollateral(credit: UInt256, snap: TokenSnapshot): UInt256 {
    return FlowCreditMarketUtils.mul(
        FlowCreditMarketUtils.mul(credit, snap.price),
        snap.risk.collateralFactor)
}

access(all) fun effectiveDebt(debit: UInt256, snap: TokenSnapshot): UInt256 {
    return FlowCreditMarketUtils.div(
        FlowCreditMarketUtils.mul(debit, snap.price),
...
access(all) fun healthFactor(view: PositionView): UInt256 {
    ...
    for t in view.balances.keys {
        ...
        if b.direction == FlowCreditMarket.BalanceDirection.Credit {
            let trueBal = FlowCreditMarket.scaledBalanceToTrueBalance(b.scaledBalance,
...
access(all) fun maxWithdraw(
    ...
    withdrawBal: FlowCreditMarket.InternalBalance?,
    ...
    if withdrawBal == nil || withdrawBal!.direction == FlowCreditMarket.BalanceDirection.Debit {
        // withdrawing increases debt
        // solve: (effColl) / (effDebt + ΔDebt) = targetHealth
        let numerator = effColl
        let denominatorTarget = FlowCreditMarketUtils.div(numerator, targetHealth)
        let ΔDebt = denominatorTarget > effDebt ? denominatorTarget - effDebt : 0 as UInt256
        let tokens = FlowCreditMarketUtils.div(
            FlowCreditMarketUtils.mul(ΔDebt, bf),
            withdrawSnap.price)
        return tokens
    } else {
        // withdrawing reduces collateral
        let trueBal = FlowCreditMarket.scaledBalanceToTrueBalance(withdrawBal!.scaledBalance,
                      interestIndex: withdrawSnap.creditIndex)
        let maxPossible = trueBal
        // solve: (effColl - ΔColl) / effDebt = targetHealth
        let requiredColl = FlowCreditMarketUtils.mul(effDebt, targetHealth)
        if effColl <= requiredColl {
            return 0
        }
        let ΔCollEff = effColl - requiredColl
        let ΔTokens = FlowCreditMarketUtils.div(
            FlowCreditMarketUtils.div(ΔCollEff, cf),
            withdrawSnap.price)
...
access(all) fun buildPositionView(pid: UInt64): FlowCreditMarket.PositionView {
    ...
    let balancesCopy: {Type: FlowCreditMarket.InternalBalance} = {}
    for t in position.balances.keys {
        let bal = position.balances[t]!
        balancesCopy[t] = FlowCreditMarket.InternalBalance(
            direction: bal.direction,
            scaledBalance: bal.scaledBalance
        )
        let ts = self._borrowUpdatedTokenState(type: t)
        snaps[t] = TokenSnapshot(
            price: FlowCreditMarketUtils.ufix64ToUInt256(self.priceOracle.price(ofToken: t)!, decimals: 18),
            credit: ts.creditInterestIndex,
            debit: ts.debitInterestIndex,
            risk: RiskParams(
                cf: FlowCreditMarketUtils.ufix64ToUInt256(self.collateralFactor[t]!, decimals: 18),
                bf: FlowCreditMarketUtils.ufix64ToUInt256(self.borrowFactor[t]!, decimals: 18),
                lb: FlowCreditMarketUtils.e18 + 50_000_000_000_000_000
            )
        )
    }
...
access(all) fun availableBalance(pid: UInt64, type: Type, pullFromTopUpSource: Bool): UFix64 {
    ...
    let snap = TokenSnapshot(
        price: FlowCreditMarketUtils.ufix64ToUInt256(self.priceOracle.price(ofToken: type)!, decimals: 18),
        credit: tokenState.creditInterestIndex,
        debit: tokenState.debitInterestIndex,
        risk: RiskParams(
            cf: FlowCreditMarketUtils.ufix64ToUInt256(self.collateralFactor[type]!, decimals: 18),
            bf: FlowCreditMarketUtils.ufix64ToUInt256(self.borrowFactor[type]!, decimals: 18),
            lb: FlowCreditMarketUtils.e18 + 50_000_000_000_000_000
        )
    )
    ...
    return FlowCreditMarketUtils.uint256ToUFix64(uintMax, decimals: 18)
}
...
access(EPosition) fun applyWithdraw(
    ...
    let uintAmount = FlowCreditMarketUtils.ufix64ToUInt256(amount, decimals: 18)
...
| P2 | Pure rate-limit & deposit-capacity maths | Simplifies rate updates (current: edges in `updateInterestRates`). | Pure funcs for rates/capacity; integrate into commands. | Refactor `updateInterestRates` (lines 300-350); add to `FlowCreditMarketMath.cdc` if separate. |
...
• **Pure helpers (FlowCreditMarketMath.cdc)**: `effectiveCollateral`, `effectiveDebt`, `healthFactor`, `maxBorrow`, `maxWithdraw`, `requiredTopUp`, `liquidationQuote`, `updateIndex`, `nextDepositCapacity`  
...
- **Pure Core**: New helpers in a dedicated section or file (e.g., lines 1000+ in FlowCreditMarket.cdc).
