...
access(all) contract FlowALP {
    ...
    /// The canonical StoragePath where the primary FlowALP Pool is stored
    ...
    /// The canonical PublicPath where the primary FlowALP Pool can be accessed publicly
    ...
    access(all) event Deposited(pid: UInt64, poolUUID: UInt64, vaultType: Type, amount: UFix64, depositedUUID: UInt64)
    access(all) event Withdrawn(pid: UInt64, poolUUID: UInt64, vaultType: Type, amount: UFix64, withdrawnUUID: UInt64)
    access(all) event Rebalanced(pid: UInt64, poolUUID: UInt64, atHealth: UFix128, amount: UFix64, fromUnder: Bool)
    ...
    /* --- NUMERIC TYPES POLICY ---
        ...
        - We convert at boundaries via FlowALPMath.toUFix128/toUFix64.
    */
    /// InternalBalance
    ///
    ...
    access(all) struct InternalBalance {
        ...
        access(contract) fun recordDeposit(amount: UFix128, tokenState: auth(EImplementation) &TokenState) {
            if self.direction == BalanceDirection.Credit {
                ...
                let scaledDeposit = FlowALP.trueBalanceToScaledBalance(amount,
                ...
                let trueBalance = FlowALP.scaledBalanceToTrueBalance(self.scaledBalance,
                ...
                if trueBalance >= amount {
                    ...
                    self.scaledBalance = FlowALP.trueBalanceToScaledBalance(updatedBalance,
                        interestIndex: tokenState.debitInterestIndex)
        ...
        access(all) fun recordWithdrawal(amount: UFix128, tokenState: &TokenState) {
            if self.direction == BalanceDirection.Debit {
                ...
                let scaledWithdrawal = FlowALP.trueBalanceToScaledBalance(amount,
                    interestIndex: tokenState.debitInterestIndex)
    ...
    access(all) struct BalanceSheet {
        ...
        init(effectiveCollateral: UFix128, effectiveDebt: UFix128) {
            ...
            self.health = FlowALP.healthComputation(effectiveCollateral: effectiveCollateral, effectiveDebt: effectiveDebt)
        }
    }
    ...
    access(all) struct LiquidationParamsView {
        ...
        init(targetHF: UFix128, paused: Bool, warmupSec: UInt64, lastUnpausedAt: UInt64?, triggerHF: UFix128, protocolFeeBps: UInt16) {
    ...
    access(all) resource InternalPosition {
        ...
        init() {
            self.balances = {}
            self.queuedDeposits <- {}
            self.targetHealth = FlowALPMath.toUFix128(1.3)
            self.minHealth = FlowALPMath.toUFix128(1.1)
            self.maxHealth = FlowALPMath.toUFix128(1.5)
    ...
    access(all) struct interface InterestCurve {
        access(all) fun interestRate(creditBalance: UFix128, debitBalance: UFix128): UFix128 {
            post {
                result <= FlowALPMath.one: "Interest rate can't exceed 100%"
            }
        }
    }
    ...
    access(all) struct TokenState {
        ...
        /// (see FlowALPMath) and are stored as UFix128 to maintain precision when converting between scaled and true
        ...
        init(interestCurve: {InterestCurve}, depositRate: UFix64, depositCapacityCap: UFix64) {
            ...
            self.creditInterestIndex = FlowALPMath.one
            self.debitInterestIndex = FlowALPMath.one
            self.currentCreditRate = FlowALPMath.one
            self.currentDebitRate = FlowALPMath.one
        ...
        access(all) fun updateInterestIndices() {
            ...
            self.creditInterestIndex = FlowALP.compoundInterestIndex(
                oldIndex: self.creditInterestIndex,
                perSecondRate: self.currentCreditRate,
                elapsedSeconds: dt
            )
            self.debitInterestIndex = FlowALP.compoundInterestIndex(
        ...
        access(all) fun updateInterestRates() {
            ...
            if self.totalCreditBalance == 0.0 as UFix128 {
                self.currentCreditRate = FlowALPMath.one  // 1.0 in fixed point (no interest)
                self.currentDebitRate = FlowALPMath.one   // 1.0 in fixed point (no interest)
                return
            }
            ...
            let insuranceRate: UFix128 = FlowALPMath.toUFix128(self.insuranceRate)
            ...
            if debitIncome >= insuranceAmount {
                creditRate = ((debitIncome - insuranceAmount) / self.totalCreditBalance) - FlowALPMath.one
            } else {
            ...
            self.currentCreditRate = FlowALP.perSecondInterestRate(yearlyRate: creditRate)
            self.currentDebitRate = FlowALP.perSecondInterestRate(yearlyRate: debitRate)
        }
    }
    ...
    access(all) struct TokenSnapshot {
        ...
        init(price: UFix128, credit: UFix128, debit: UFix128, risk: RiskParams) {
            self.price = price
    ...
    access(all) view fun healthFactor(view: PositionView): UFix128 {
        ...
        for tokenType in view.balances.keys {
            ...
            if balance.direction == BalanceDirection.Credit {
                let trueBalance = FlowALP.scaledBalanceToTrueBalance(
                    balance.scaledBalance,
                    interestIndex: snap.creditIndex
                )
                effectiveCollateralTotal = effectiveCollateralTotal + FlowALP.effectiveCollateral(credit: trueBalance, snap: snap)
            } else {
                let trueBalance = FlowALP.scaledBalanceToTrueBalance(
                    balance.scaledBalance,
                    interestIndex: snap.debitIndex
                )
                effectiveDebtTotal = effectiveDebtTotal + FlowALP.effectiveDebt(debit: trueBalance, snap: snap)
            }
        }
    ...
    access(all) view fun maxWithdraw(
        ...
        let preHealth = FlowALP.healthFactor(view: view)
        ...
        if withdrawBal == nil || withdrawBal!.direction == BalanceDirection.Debit {
            ...
            let deltaDebt = denominatorTarget > effectiveDebtTotal ? denominatorTarget - effectiveDebtTotal : FlowALPMath.zero
    ...
    access(all) resource Pool {
        ...
        init(defaultToken: Type, priceOracle: {DeFiActions.PriceOracle}) {
            ...
            self.liquidationTargetHF = FlowALPMath.toUFix128(1.05)
        ...
        access(all) fun getLiquidationParams(): FlowALP.LiquidationParamsView {
            return FlowALP.LiquidationParamsView(
                ...
                triggerHF: FlowALPMath.one, // 1.0e24
        ...
        access(all) fun availableBalance(pid: UInt64, type: Type, pullFromTopUpSource: Bool): UFix64 {
            if self.debugLogging { log("    [CONTRACT] availableBalance(pid: \(pid), type: \(type.contractName!), pullFromTopUpSource: \(pullFromTopUpSource))") }
            ...
            if pullFromTopUpSource && position.topUpSource != nil {
                ...
                if self.debugLogging { log("    [CONTRACT] Calling to fundsAvailableAboveTargetHealthAfterDepositing with sourceAmount \(sourceAmount) and targetHealth \(position.minHealth)") }
            ...
            let snap = FlowALP.TokenSnapshot(
                price: FlowALPMath.toUFix128(self.priceOracle.price(ofToken: type)!),
                credit: tokenState.creditInterestIndex,
                debit: tokenState.debitInterestIndex,
                risk: FlowALP.RiskParams(
                    cf: FlowALPMath.toUFix128(self.collateralFactor[type]!),
                    bf: FlowALPMath.toUFix128(self.borrowFactor[type]!),
                    lb: FlowALPMath.toUFix128(self.liquidationBonus[type]!)
                )
            )

            let withdrawBal = view.balances[type]
            let uintMax = FlowALP.maxWithdraw(
                view: view,
                withdrawSnap: snap,
        ...
        access(all) fun positionHealth(pid: UInt64): UFix128 {
            ...
            for type in position.balances.keys {
                ...
                let collateralFactor = FlowALPMath.toUFix128(self.collateralFactor[type]!)
                let borrowFactor = FlowALPMath.toUFix128(self.borrowFactor[type]!)
                let price = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: type)!)
                if balance.direction == BalanceDirection.Credit {
                    let trueBalance = FlowALP.scaledBalanceToTrueBalance(balance.scaledBalance,
                        interestIndex: tokenState.creditInterestIndex)
            ...
            return FlowALP.healthComputation(effectiveCollateral: effectiveCollateral, effectiveDebt: effectiveDebt)
        }
        ...
        access(all) fun getPositionDetails(pid: UInt64): PositionDetails {
            ...
            for type in position.balances.keys {
                ...
                let trueBalance = balance.direction == BalanceDirection.Credit
                    ? FlowALP.scaledBalanceToTrueBalance(balance.scaledBalance, interestIndex: tokenState.creditInterestIndex)
                    : FlowALP.scaledBalanceToTrueBalance(balance.scaledBalance, interestIndex: tokenState.debitInterestIndex)
        ...
        access(all) fun quoteLiquidation(pid: UInt64, debtType: Type, seizeType: Type): FlowALP.LiquidationQuote {
            pre {
            ...
            let view = self.buildPositionView(pid: pid)
            let health = FlowALP.healthFactor(view: view)
            if health >= FlowALPMath.one {
                return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: health)
            }
            // Build snapshots
            ...
            let debtSnap = FlowALP.TokenSnapshot(
                price: FlowALPMath.toUFix128(self.priceOracle.price(ofToken: debtType)!),
                credit: debtState.creditInterestIndex,
                debit: debtState.debitInterestIndex,
                risk: FlowALP.RiskParams(
                    cf: FlowALPMath.toUFix128(self.collateralFactor[debtType]!),
                    bf: FlowALPMath.toUFix128(self.borrowFactor[debtType]!),
                    lb: FlowALPMath.toUFix128(lbDebtUFix)
                )
            )
            ...
            let seizeSnap = FlowALP.TokenSnapshot(
                price: FlowALPMath.toUFix128(self.priceOracle.price(ofToken: seizeType)!),
                credit: seizeState.creditInterestIndex,
                debit: seizeState.debitInterestIndex,
                risk: FlowALP.RiskParams(
                    cf: FlowALPMath.toUFix128(self.collateralFactor[seizeType]!),
                    bf: FlowALPMath.toUFix128(self.borrowFactor[seizeType]!),
                    lb: FlowALPMath.toUFix128(lbSeizeUFix)
                )
            )
            ...
            var lbTUFix: UFix64 = 0.05
                ...
                let snap = FlowALP.TokenSnapshot(
                    price: FlowALPMath.toUFix128(self.priceOracle.price(ofToken: t)!),
                    credit: st.creditInterestIndex,
                    debit: st.debitInterestIndex,
                    risk: FlowALP.RiskParams(
                        cf: FlowALPMath.toUFix128(self.collateralFactor[t]!),
                        bf: FlowALPMath.toUFix128(self.borrowFactor[t]!),
                        lb: FlowALPMath.toUFix128(lbTUFix)
                    )
                )
                if b.direction == BalanceDirection.Credit {
                    let trueBal = FlowALP.scaledBalanceToTrueBalance(b.scaledBalance, interestIndex: snap.creditIndex)
                    if t == seizeType {
                        trueCollateralSeize = trueBal
                    }
                    effColl = effColl + FlowALP.effectiveCollateral(credit: trueBal, snap: snap)
                } else {
                    let trueBal = FlowALP.scaledBalanceToTrueBalance(b.scaledBalance, interestIndex: snap.debitIndex)
                    if t == debtType {
                        trueDebt = trueBal
                    }
                    effDebt = effDebt + FlowALP.effectiveDebt(debit: trueBal, snap: snap)
                }
            }
            ...
            if effDebt == 0.0 as UFix128 { // no debt
                return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: UFix128.max)
            }
            ...
            let effDebtNew = FlowALPMath.div(effColl, target)
            if effDebt <= effDebtNew {
                return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: target)
            }
            ...
            if effDebt == 0.0 as UFix128 || effColl / effDebt >= target {
                return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: effColl / effDebt)
            }

            // Derived formula with positive denominator: u = (t * effDebt - effColl) / (t - (1 + LB) * CF)
            let num = effDebt * target - effColl
            let denomFactor = target - ((FlowALPMath.one + LB) * CF)
            if denomFactor <= FlowALPMath.zero {
                // Impossible target, return 0
                return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: health)
            }
            var repayTrueU128 = FlowALPMath.div(num * BF, Pd * denomFactor)
            if repayTrueU128 > trueDebt {
                repayTrueU128 = trueDebt
            }
            let u = FlowALPMath.div(repayTrueU128 * Pd, BF)
            var seizeTrueU128 = FlowALPMath.div(u * (FlowALPMath.one + LB), Pc)
            if seizeTrueU128 > trueCollateralSeize {
                seizeTrueU128 = trueCollateralSeize
                let uAllowed = FlowALPMath.div(seizeTrueU128 * Pc, (FlowALPMath.one + LB))
                repayTrueU128 = FlowALPMath.div(uAllowed * BF, Pd)
                if repayTrueU128 > trueDebt {
                    repayTrueU128 = trueDebt
                }
            }
            let repayExact = FlowALPMath.toUFix64RoundUp(repayTrueU128)
            let seizeExact = FlowALPMath.toUFix64RoundUp(seizeTrueU128)
            let repayEff = FlowALPMath.div(repayTrueU128 * Pd, BF)
            let seizeEff = seizeTrueU128 * (Pc * CF)
            let newEffColl = effColl > seizeEff ? effColl - seizeEff : FlowALPMath.zero
            let newEffDebt = effDebt > repayEff ? effDebt - repayEff : FlowALPMath.zero
            let newHF = newEffDebt == FlowALPMath.zero ? UFix128.max : FlowALPMath.div(newEffColl * FlowALPMath.one, newEffDebt)
            ...
            let uAllowedMax = FlowALPMath.div(trueCollateralSeize * Pc, (FlowALPMath.one + LB))
            var repayCapBySeize = FlowALPMath.div(uAllowedMax * BF, Pd)
                ...
                if repayCapBySeize == FlowALPMath.zero || trueCollateralSeize == FlowALPMath.zero {
                    return FlowALP.LiquidationQuote(requiredRepay: 0.0, seizeType: seizeType, seizeAmount: 0.0, newHF: health)
                }

                // Discrete bounded search over repay in [1..repayCapBySeize]
                // Use up to 16 steps to balance precision and cost
                let stepsU: UFix128 = FlowALPMath.toUFix128(16.0)
                var step: UFix128 = repayCapBySeize / stepsU
                if step == FlowALPMath.zero { step = FlowALPMath.one }

                var r: UFix128 = step
                while r <= repayCapBySeize {
                    // Compute S for this R under pricing relation, capped by available collateral
                    let uForR = FlowALPMath.div(r * Pd, BF)
                    var sForR = FlowALPMath.div(uForR * (FlowALPMath.one + LB), Pc)
                    if sForR > trueCollateralSeize { sForR = trueCollateralSeize }

                    // Compute resulting HF
                    let repayEffC = FlowALPMath.div(r * Pd, BF)
                    let seizeEffC = sForR * (Pc * CF)
                    let newEffCollC = effColl > seizeEffC ? effColl - seizeEffC : FlowALPMath.zero
                    let newEffDebtC = effDebt > repayEffC ? effDebt - repayEffC : FlowALPMath.zero
                    let newHFC = newEffDebtC == FlowALPMath.zero ? UFix128.max : FlowALPMath.div(newEffCollC * FlowALPMath.one, newEffDebtC)
                ...
                let uForR2 = FlowALPMath.div(rCap * Pd, BF)
                var sForR2 = FlowALPMath.div(uForR2 * (FlowALPMath.one + LB), Pc)
                if sForR2 > trueCollateralSeize { sForR2 = trueCollateralSeize }
                let repayEffC2 = FlowALPMath.div(rCap * Pd, BF)
                let seizeEffC2 = sForR2 * (Pc * CF)
                let newEffCollC2 = effColl > seizeEffC2 ? effColl - seizeEffC2 : FlowALPMath.zero
                let newEffDebtC2 = effDebt > repayEffC2 ? effDebt - repayEffC2 : FlowALPMath.zero
                let newHFC2 = newEffDebtC2 == FlowALPMath.zero ? UFix128.max : FlowALPMath.div(newEffCollC2 * FlowALPMath.one, newEffDebtC2)
                if newHFC2 > bestHF {
                    bestHF = newHFC2
                    bestRepayTrue = rCap
                ...
                if bestHF > health && bestRepayTrue > FlowALPMath.zero && bestSeizeTrue > FlowALPMath.zero {
                    let repayExactBest = FlowALPMath.toUFix64RoundUp(bestRepayTrue)
                    let seizeExactBest = FlowALPMath.toUFix64RoundUp(bestSeizeTrue)
                    log("[LIQ][QUOTE][FALLBACK][SEARCH] repayExact=\(repayExactBest) seizeExact=\(seizeExactBest)")
                    return FlowALP.LiquidationQuote(requiredRepay: repayExactBest, seizeType: seizeType, seizeAmount: seizeExactBest, newHF: bestHF)
                }
            ...
            log("[LIQ][QUOTE] repayExact=\(repayExact) seizeExact=\(seizeExact) trueCollateralSeize=\(FlowALPMath.toUFix64Round(trueCollateralSeize))")
            return FlowALP.LiquidationQuote(requiredRepay: repayExact, seizeType: seizeType, seizeAmount: seizeExact, newHF: newHF)
        }
        ...
        access(all) fun liquidateRepayForSeize(
            ...
            let position = self._borrowPosition(pid: pid)
            let debtState = self._borrowUpdatedTokenState(type: debtType)
            let repayUint = FlowALPMath.toUFix128(quote.requiredRepay)
            ...
            let seizeState = self._borrowUpdatedTokenState(type: seizeType)
            let seizeUint = FlowALPMath.toUFix128(quote.seizeAmount)
            ...
            let hfTolerance: UFix128 = FlowALPMath.toUFix128(0.00001)
        ...
        access(all) fun liquidateViaDex(
            ...
            let health = self.positionHealth(pid: pid)
            assert(health < FlowALPMath.one, message: "Position not liquidatable")
        ...
        access(self) fun internalSeize(pid: UInt64, tokenType: Type, amount: UFix64): @{FungibleToken.Vault} {
            ...
            let seizeUint = FlowALPMath.toUFix128(amount)
        ...
        access(self) fun internalRepay(pid: UInt64, from: @{FungibleToken.Vault}): UFix64 {
            ...
            let position = self._borrowPosition(pid: pid)
            let debtState = self._borrowUpdatedTokenState(type: debtType)
            let repayUint = FlowALPMath.toUFix128(amount)
            if position.balances[debtType] == nil {
                position.balances[debtType] = InternalBalance(direction: BalanceDirection.Debit, scaledBalance: 0.0 as UFix128)
            }
            position.balances[debtType]!.recordDeposit(amount: repayUint, tokenState: debtState)
        ...
        access(self) fun computeAdjustedBalancesAfterWithdrawal(
            ...
            let withdrawAmountU = FlowALPMath.toUFix128(withdrawAmount)
            let withdrawPrice2 = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: withdrawType)!)
            let withdrawBorrowFactor2 = FlowALPMath.toUFix128(self.borrowFactor[withdrawType]!)
            ...
            let maybeBalance = position.balances[withdrawType]
                if maybeBalance == nil || maybeBalance!.direction == BalanceDirection.Debit {
                    ...
                    effectiveDebtAfterWithdrawal = balanceSheet.effectiveDebt +
                        FlowALPMath.div(withdrawAmountU * withdrawPrice2, withdrawBorrowFactor2)
                } else {
                    ...
                    let trueCollateral = FlowALP.scaledBalanceToTrueBalance(collateralBalance,
                        interestIndex: withdrawTokenState.creditInterestIndex
                    )
                    let collateralFactor = FlowALPMath.toUFix128(self.collateralFactor[withdrawType]!)
                    if trueCollateral >= withdrawAmountU {
                        ...
                        effectiveDebtAfterWithdrawal = balanceSheet.effectiveDebt +
                            FlowALPMath.div((withdrawAmountU - trueCollateral) * withdrawPrice2, withdrawBorrowFactor2)
                        effectiveCollateralAfterWithdrawal = balanceSheet.effectiveCollateral -
        ...
        access(self) fun computeRequiredDepositForHealth(
            ...
            var healthAfterWithdrawal = FlowALP.healthComputation(
                effectiveCollateral: effectiveCollateralAfterWithdrawal,
                effectiveDebt: effectiveDebtAfterWithdrawal
            )
            if self.debugLogging { log("    [CONTRACT] healthAfterWithdrawal: \(healthAfterWithdrawal)") }
            ...
            var debtTokenCount: UFix128 = FlowALPMath.zero
            let depositPrice = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: depositType)!)
            let depositBorrowFactor = FlowALPMath.toUFix128(self.borrowFactor[depositType]!)
            let withdrawBorrowFactor = FlowALPMath.toUFix128(self.borrowFactor[withdrawType]!)
            ...
            if maybeBalance?.direction == BalanceDirection.Debit {
                ...
                let trueDebt = FlowALP.scaledBalanceToTrueBalance(debtBalance,
                    interestIndex: depositTokenState.debitInterestIndex
                )
                let debtEffectiveValue = FlowALPMath.div(depositPrice * trueDebt, depositBorrowFactor)
            ...
            var effectiveDebtAfterPayment: UFix128 = FlowALPMath.zero
                ...
                let potentialHealth = FlowALP.healthComputation(
                    effectiveCollateral: effectiveCollateralAfterWithdrawal,
                ...
                if potentialHealth >= targetHealth {
                    ...
                    let requiredEffectiveDebt = effectiveDebtAfterWithdrawal - FlowALPMath.div(
                            effectiveCollateralAfterWithdrawal,
                            targetHealth
                        )

                    // The amount of the token to pay back, in units of the token.
                    let paybackAmount = FlowALPMath.div(requiredEffectiveDebt * depositBorrowFactor, depositPrice)
                    ...
                    debtTokenCount = FlowALPMath.div(trueDebt, depositPrice)
            ...
            let depositCollateralFactor = FlowALPMath.toUFix128(self.collateralFactor[depositType]!)
            var requiredEffectiveCollateral = healthChangeU * effectiveDebtAfterWithdrawal
            requiredEffectiveCollateral = FlowALPMath.div(requiredEffectiveCollateral, depositCollateralFactor)

            // The amount of the token to deposit, in units of the token.
            let collateralTokenCount = FlowALPMath.div(requiredEffectiveCollateral, depositPrice)
            if self.debugLogging {
        ...
        access(all) fun fundsAvailableAboveTargetHealthAfterDepositing(
            ...
            if self.debugLogging { log("    [CONTRACT] fundsAvailableAboveTargetHealthAfterDepositing(pid: \(pid), withdrawType: \(withdrawType.contractName!), targetHealth: \(targetHealth), depositType: \(depositType.contractName!), depositAmount: \(depositAmount))") }
        ...
        access(self) fun computeAdjustedBalancesAfterDeposit(
            ...
            let depositAmountCasted = FlowALPMath.toUFix128(depositAmount)
            let depositPriceCasted = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: depositType)!)
            let depositBorrowFactorCasted = FlowALPMath.toUFix128(self.borrowFactor[depositType]!)
            let depositCollateralFactorCasted = FlowALPMath.toUFix128(self.collateralFactor[depositType]!)
            let maybeBalance = position.balances[depositType]
                if maybeBalance == nil || maybeBalance!.direction == BalanceDirection.Credit {
                    ...
                    if trueDebt >= depositAmountCasted {
                        ...
                        effectiveDebtAfterDeposit = balanceSheet.effectiveDebt -
                            FlowALPMath.div(depositAmountCasted * depositPriceCasted, depositBorrowFactorCasted)
                    } else {
                        // The deposit will wipe out all of the debt, and create some collateral.
                        // TODO - validate if this should deal with withdrawType or depositType
                        effectiveDebtAfterDeposit = balanceSheet.effectiveDebt -
                            FlowALPMath.div(trueDebt * depositPriceCasted, depositBorrowFactorCasted)
                        effectiveCollateralAfterDeposit = balanceSheet.effectiveCollateral +
        ...
        access(self) fun computeAvailableWithdrawal(
            ...
            var healthAfterDeposit = FlowALP.healthComputation(
                effectiveCollateral: effectiveCollateralAfterDeposit,
                effectiveDebt: effectiveDebtAfterDeposit
            )
            if self.debugLogging { log("    [CONTRACT] healthAfterDeposit: \(healthAfterDeposit)") }
            ...
            var collateralTokenCount: UFix128 = FlowALPMath.zero

            let withdrawPrice = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: withdrawType)!)
            let withdrawCollateralFactor = FlowALPMath.toUFix128(self.collateralFactor[withdrawType]!)
            let withdrawBorrowFactor = FlowALPMath.toUFix128(self.borrowFactor[withdrawType]!)
            ...
            if maybeBalance?.direction == BalanceDirection.Credit {
                ...
                let trueCredit = FlowALP.scaledBalanceToTrueBalance(creditBalance,
                    interestIndex: withdrawTokenState.creditInterestIndex
                )
            ...
            let availableTokenCount = FlowALPMath.div(FlowALPMath.div(availableEffectiveValue, withdrawCollateralFactor), withdrawPrice)
                    ...
                    var availableDebtIncrease = FlowALPMath.div(effectiveCollateralAfterDeposit, targetHealth) - effectiveDebtAfterDeposit
                    let availableTokens = FlowALPMath.div(availableDebtIncrease * withdrawBorrowFactor, withdrawPrice)
                    if self.debugLogging {
        ...
        access(all) fun healthAfterDeposit(pid: UInt64, type: Type, amount: UFix64): UFix128 {
            ...
            var effectiveCollateralIncrease: UFix128 = FlowALPMath.zero
            var effectiveDebtDecrease: UFix128 = FlowALPMath.zero

            let amountU = FlowALPMath.toUFix128(amount)
            let price = FlowALPMath.toUFix128(self.priceOracle.price(ofToken: type)!)
            let collateralFactor = FlowALPMath.toUFix128(self.collateralFactor[type]!)
            let borrowFactor = FlowALPMath.toUFix128(self.borrowFactor[type]!)
            if position.balances[type] == nil || position.balances[type]!.direction == BalanceDirection.Credit {
                ...
                if trueDebt >= amountU {
                    ...
                    // just need to account for the debt decrease.
                    effectiveDebtDecrease = FlowALPMath.div(amountU * price, borrowFactor)
                } else {
        ...
        access(all) fun healthAfterWithdrawal(pid: UInt64, type: Type, amount: UFix64): UFix128 {
            ...
            var effectiveCollateralDecrease: UFix128 = FlowALPMath.zero
            var effectiveDebtIncrease: UFix128 = FlowALPMath.zero
            ...
            if position.balances[type] == nil || position.balances[type]!.direction == BalanceDirection.Debit {
                ...
                effectiveDebtIncrease = FlowALPMath.div(amountU * price, borrowFactor)
            } else {
                ...
                if trueCredit >= amountU {
                    ...
                    effectiveDebtIncrease = FlowALPMath.div((amountU - trueCredit) * price, borrowFactor)
        ...
        access(EPosition) fun depositAndPush(pid: UInt64, from: @{FungibleToken.Vault}, pushToDrawDownSink: Bool) {
            ...
            if self.debugLogging { log("    [CONTRACT] depositAndPush(pid: \(pid), pushToDrawDownSink: \(pushToDrawDownSink))") }
            ...
            let uintDepositAmount = FlowALPMath.toUFix128(depositAmount)
        ...
        access(EPosition) fun withdrawAndPull(
            ...
            if self.debugLogging { log("    [CONTRACT] withdrawAndPull(pid: \(pid), type: \(type.identifier), amount: \(amount), pullFromTopUpSource: \(pullFromTopUpSource))") }
            if amount == 0.0 {
        ...
        access(EPosition) view fun readTargetHealth(pid: UInt64): UFix128 {
            let pos: auth(EImplementation) &InternalPosition = self._borrowPosition(pid: pid)
        ...
        access(EPosition) fun writeTargetHealth(pid: UInt64, targetHealth: UFix128) {
            let pos: auth(EImplementation) &InternalPosition = self._borrowPosition(pid: pid)
            assert(targetHealth >= pos.minHealth, message: "targetHealth must be ≥ minHealth")
            assert(targetHealth <= pos.maxHealth, message: "targetHealth must be ≤ maxHealth")
            pos.setTargetHealth(targetHealth)
        }
        access(EPosition) fun writeMinHealth(pid: UInt64, minHealth: UFix128) {
            let pos: auth(EImplementation) &InternalPosition = self._borrowPosition(pid: pid)
            assert(minHealth <= pos.targetHealth, message: "minHealth must be ≤ targetHealth")
            pos.setMinHealth(minHealth)
        }
        access(EPosition) fun writeMaxHealth(pid: UInt64, maxHealth: UFix128) {
            let pos: auth(EImplementation) &InternalPosition = self._borrowPosition(pid: pid)
            assert(maxHealth >= pos.targetHealth, message: "maxHealth must be ≥ targetHealth")
        ...
        access(EGovernance) fun setLiquidationParams(
            ...
            if targetHF != nil {
                assert(targetHF! > FlowALPMath.one, message: "targetHF must be > 1.0")
        ...
        access(EGovernance) fun setInsuranceRate(tokenType: Type, insuranceRate: UFix64) {
            pre {
                self.globalLedger[tokenType] != nil: "Unsupported token type"
                insuranceRate >= 0.0 && insuranceRate <= 1.0: "insuranceRate must be between 0 and 1"
            }
        ...
        access(EGovernance) fun setDepositLimitFraction(tokenType: Type, fraction: UFix64) {
            pre {
                self.globalLedger[tokenType] != nil: "Unsupported token type"
                fraction > 0.0 && fraction <= 1.0: "fraction must be in (0,1]"
            }
            let tsRef = &self.globalLedger[tokenType] as auth(EImplementation) &TokenState?
                ?? panic("Invariant: token state missing")
        ...
        access(EPosition) fun rebalancePosition(pid: UInt64, force: Bool) {
            if self.debugLogging { log("    [CONTRACT] rebalancePosition(pid: \(pid), force: \(force))") }
            let position = self._borrowPosition(pid: pid)
            let balanceSheet = self._getUpdatedBalanceSheet(pid: pid)
            ...
            if balanceSheet.health < position.targetHealth {
                ...
                if position.topUpSource != nil {
                    ...
                    if self.debugLogging { log("    [CONTRACT] idealDeposit: \(idealDeposit)") }
                ...
                if position.drawDownSink != nil {
                    ...
                    if self.debugLogging { log("    [CONTRACT] idealWithdrawal: \(idealWithdrawal)") }
                    ...
                    if sinkAmount > 0.0 && sinkType == self.defaultToken { // second conditional included for sake of tracer bullet
                        ...
                        let uintSinkAmount = FlowALPMath.toUFix128(sinkAmount)
                        position.balances[self.defaultToken]!.recordWithdrawal(amount: uintSinkAmount, tokenState: tokenState)
                        let sinkVault <- FlowALP._borrowMOETMinter().mintTokens(amount: sinkAmount)
        ...
        access(self) fun _getUpdatedBalanceSheet(pid: UInt64): BalanceSheet {
            ...
            for type in position.balances.keys {
                ...
                if balance.direction == BalanceDirection.Credit {
                    ...
                    let convertedPrice = FlowALPMath.toUFix128(priceOracle.price(ofToken: type)!)
                    let value = convertedPrice * trueBalance

                    let convertedCollateralFactor = FlowALPMath.toUFix128(self.collateralFactor[type]!)
                    ...
                    let convertedBorrowFactor = FlowALPMath.toUFix128(self.borrowFactor[type]!)
                    effectiveDebt = effectiveDebt + FlowALPMath.div(value, convertedBorrowFactor)
                }
            }
        ...
        access(all) fun buildPositionView(pid: UInt64): FlowALP.PositionView {
            ...
            let balancesCopy: {Type: FlowALP.InternalBalance} = position.copyBalances()
            for t in position.balances.keys {
                let tokenState = self._borrowUpdatedTokenState(type: t)
                snaps[t] = FlowALP.TokenSnapshot(
                    price: FlowALPMath.toUFix128(self.priceOracle.price(ofToken: t)!),
                    credit: tokenState.creditInterestIndex,
                    debit: tokenState.debitInterestIndex,
                    risk: FlowALP.RiskParams(
                        cf: FlowALPMath.toUFix128(self.collateralFactor[t]!),
                        bf: FlowALPMath.toUFix128(self.borrowFactor[t]!),
                        lb: FlowALPMath.toUFix128(self.liquidationBonus[t]!)
                    )
                )
            }
    ...
    access(all) resource PoolFactory {
        ...
        access(all) fun createPool(defaultToken: Type, priceOracle: {DeFiActions.PriceOracle}) {
            pre {
                FlowALP.account.storage.type(at: FlowALP.PoolStoragePath) == nil:
                "Storage collision - Pool has already been created & saved to \(FlowALP.PoolStoragePath)"
            }
            let pool <- create Pool(defaultToken: defaultToken, priceOracle: priceOracle)
            FlowALP.account.storage.save(<-pool, to: FlowALP.PoolStoragePath)
            let cap = FlowALP.account.capabilities.storage.issue<&Pool>(FlowALP.PoolStoragePath)
            FlowALP.account.capabilities.unpublish(FlowALP.PoolPublicPath)
            FlowALP.account.capabilities.publish(cap, at: FlowALP.PoolPublicPath)
        }
    }
    ...
    access(all) struct Position {
        ...
        access(all) fun setTargetHealth(targetHealth: UFix64) {
            let pool = self.pool.borrow()!
            let uint = FlowALPMath.toUFix128(targetHealth)
        ...
        access(all) fun setMinHealth(minHealth: UFix64) {
            let pool = self.pool.borrow()!
            let uint = FlowALPMath.toUFix128(minHealth)
        ...
        access(all) fun setMaxHealth(maxHealth: UFix64) {
            let pool = self.pool.borrow()!
            let uint = FlowALPMath.toUFix128(maxHealth)
    ...
    access(all) struct PositionDetails {
        ...
        init(balances: [PositionBalance], poolDefaultToken: Type, defaultTokenAvailableBalance: UFix64, health: UFix128) {
            self.balances = balances
    ...
    access(all) view fun healthComputation(effectiveCollateral: UFix128, effectiveDebt: UFix128): UFix128 {
        if effectiveDebt == 0.0 as UFix128 {
            ...
            return 0.0 as UFix128
        } else if FlowALPMath.div(effectiveDebt, effectiveCollateral) == 0.0 as UFix128 {
            // Negligible debt relative to collateral: treat as infinite
            return UFix128.max
        } else {
            return FlowALPMath.div(effectiveCollateral, effectiveDebt)
        }
    }
    ...
    access(all) view fun perSecondInterestRate(yearlyRate: UFix128): UFix128 {
        ...
        let perSecondScaledValue = FlowALPMath.div(yearlyRate, secondsInYear)
        assert(perSecondScaledValue < UFix128.max, message: "Per-second interest rate \(perSecondScaledValue) is too high")
        return perSecondScaledValue + FlowALPMath.one
    }
    ...
    access(all) view fun scaledBalanceToTrueBalance(_ scaled: UFix128, interestIndex: UFix128): UFix128 {
        ...
        return FlowALPMath.div(scaled * interestIndex, FlowALPMath.one)
    }
    ...
    access(all) view fun trueBalanceToScaledBalance(_ trueBalance: UFix128, interestIndex: UFix128): UFix128 {
        ...
        return FlowALPMath.div(trueBalance * FlowALPMath.one, interestIndex)
    }
