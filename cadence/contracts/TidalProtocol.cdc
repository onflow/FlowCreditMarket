...
access(all) contract TidalProtocol {
	...
	access(all) resource Pool {
		...
		access(self) fun computeAdjustedBalancesAfterWithdrawal(
			...
			var collateral = balanceSheet.effectiveCollateral
			var debt = balanceSheet.effectiveDebt

			if withdrawAmount == 0.0 {
				return AdjustedBalances(collateral: collateral, debt: debt)
			}
			...
			if maybeBalance == nil || maybeBalance!.direction == BalanceDirection.Debit {
				...
				let credit = TidalProtocol.scaledBalanceToTrueBalance(scaledBalance: scaled, interestIndex: tokenState.creditInterestIndex)

				if credit >= withdrawAmount {
					collateral = collateral - (withdrawAmount * price * cf)
				} else {
					debt = debt + ((withdrawAmount - credit) * price / bf)
					collateral = collateral - (credit * price * cf)
				}
			}
		...
		access(self) fun computeRequiredDepositForHealth(
			...
			if maybeBalance != nil && maybeBalance!.direction == BalanceDirection.Debit {
				...
				debtTokenCount = debtAmount
				debt = debt - debtValue
				health = potentialHealth
			}

			let requiredCollateral = (targetHealth - health) * debt
		...
		access(self) fun computeAdjustedBalancesAfterDeposit(
			...
			var effectiveCollateral = balanceSheet.effectiveCollateral
			var effectiveDebt = balanceSheet.effectiveDebt

			if depositAmount == 0.0 {
				return AdjustedBalances(collateral: effectiveCollateral, debt: effectiveDebt)
			}

			let depositTokenPrice = self.priceOracle.price(ofToken: depositType)!
			...
			if position.balances[depositType] == nil || position.balances[depositType]!.direction == BalanceDirection.Credit {
				...
				(depositAmount * depositTokenPrice * collateralFactor)
			} else {
				...
				if trueDebt >= depositAmount {
					...
					(depositAmount * depositTokenPrice / borrowFactor)
				} else {
					effectiveDebt = effectiveDebt -
					(trueDebt * depositTokenPrice / borrowFactor)
					effectiveCollateral = effectiveCollateral +
					((depositAmount - trueDebt) * depositTokenPrice * collateralFactor)
				}
			}
		...
		access(self) fun computeAvailableWithdrawal(
			...
			var collateralTokenCount: UFix64 = 0.0
			var adjustedCollateral = effectiveCollateral

			let maybeBalance = position.balances[withdrawType]
			if maybeBalance != nil {
				let withdrawBalance = maybeBalance!
				if withdrawBalance.direction == BalanceDirection.Credit {
					...
					let trueCredit = TidalProtocol.scaledBalanceToTrueBalance(
						scaledBalance: withdrawBalance.scaledBalance,
					...
					let potentialHealth = TidalProtocol.healthComputation(
						effectiveCollateral: adjustedCollateral - collateralEffectiveValue,
						effectiveDebt: effectiveDebt
					)
					...
					if potentialHealth <= targetHealth {
						...
						? adjustedCollateral
						: availableHealth * effectiveDebt

						return availableEffectiveValue / self.collateralFactor[withdrawType]! / self.priceOracle.price(ofToken: withdrawType)!
					}
	...
	access(all) view fun healthComputation(effectiveCollateral: UFix64, effectiveDebt: UFix64): UFix64 {
		if effectiveCollateral == 0.0 {
			return 0.0
		} else if effectiveDebt == 0.0 || (effectiveDebt / effectiveCollateral) == 0.0 {
