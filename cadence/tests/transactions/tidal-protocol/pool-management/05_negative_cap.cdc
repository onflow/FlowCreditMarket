...
transaction() {
    ...
    prepare(nonAdmin: auth(Capabilities) &Account) {
        ...
        let badGovCap: Capability<auth(TidalProtocol.EGovernance) &TidalProtocol.PoolFactory> =
            nonAdmin.capabilities.storage.issue<auth(TidalProtocol.EGovernance) &TidalProtocol.PoolFactory>(
