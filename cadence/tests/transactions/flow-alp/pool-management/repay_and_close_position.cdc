...
//
// The MockFlowALPConsumer.PositionWrapper provides the necessary
...
transaction(positionWrapperPath: StoragePath) {
    ...
    let positionRef: auth(FungibleToken.Withdraw) &FlowALP.Position
    ...
    prepare(borrower: auth(BorrowValue) &Account) {
        ...
        let wrapperRef = borrower.storage.borrow<&MockFlowALPConsumer.PositionWrapper>(
