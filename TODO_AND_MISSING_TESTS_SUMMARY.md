...
2. **`withdraw_reserve.cdc`** (Lines 16, 22)
   ...
   - TODO: Once TidalProtocol.Pool exposes a withdrawReserve function
   - Currently using a workaround that creates empty vaults

### Contract Files
1. **`TidalProtocol.cdc`** (Line 1389)
...
2. **`TidalProtocol.cdc`** (Line 1452)
   - TODO: Consider making Position a resource given its critical role

## Missing Tests (From Test Plan)

Based on `TidalProtocol_TestPlan.md`, all planned tests have been implemented:
...
**Location**: `TidalProtocol.cdc` - Pool contract missing helper method  
